...
import com.xm4399.util.{CollectUtil, ExceptionUtil, JDBCUtil, JDBCErrorLogAndCheckUtil, JDBCOnlineUtil, KuduUtil_2}
...
object CheckMysqlAndKudu {
  ...
  def checkSubTable(jobID : String, spark : SparkSession, kuduClient: KuduClient, address : String, username : String, password : String,
    ...
    if (! collectUtil.isSameForTwoLists(mysqlPriKeyList, kuduPriKeyList)){
      ...
      new JDBCErrorLogAndCheckUtil().insertErroeInfo(jobID, "CheckData", "mysql表和kudu表主键不对应")
      return false
    }
    ...
    def circularProcess(item : Row) : Boolean = {
      ...
      val kuduOneRowFieldsAndValue = KuduUtil.getkuduRowMap(kuduClient, kuduTable, priKeyMap)
