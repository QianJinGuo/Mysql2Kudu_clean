...
object NewFullPullTest {
  def main(args: Array[String]): Unit = {
    ...
    val tableStru = GetTableStru.getTableStru(dbName, tableName, isSubTable) //JdbcTest.getDBTableStru
    ...
    if ("true".equals(isSubTable)) {
      val subTableNameList = ListAllSubTableName.listAllSmallTableName(args(0), args(1)).asScala
      for(oneSubTableName <- subTableNameList){
        println("正在加载表   "+ oneSubTableName +">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
        ReadMysqlSubTable2Kudu(dbName, tableName, oneSubTableName,fieldNameArr)
  ...
  def ReadMysqlSubTable2Kudu(dbName: String, tableName : String, oneSubTableName: String, array: Array[String]): Unit = {
    ...
    val jdbcDF = spark.read
      ...
      .load()
      .withColumn("table_name",lit(oneSubTableName)) //分表的情况下,kudu表相比于mysql分表,多了table_name这个主键
